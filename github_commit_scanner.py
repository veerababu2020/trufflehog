...
PAT_TOKEN  = "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"                      # leave empty for public repos (60 req/hr)
                                     # paste your PAT here for 5000 req/hr

INPUT_CSV  = "trufflehog.csv"       # columns: repo, commit
...
def read_csv(path: str) -> list:
    ...
    with open(fp, newline="", encoding="utf-8") as f:
        ...
        if "repo" not in fields or "commit" not in fields:
            print("[!] CSV must have 'repo' and 'commit' columns.")
            sys.exit(1)

        for row in reader:
            repo_raw   = (row.get("repo")   or "").strip()
            commit_sha = (row.get("commit") or "").strip()
